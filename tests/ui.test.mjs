...
export function register(test, equal, deepEqual) {
  test('every Phase 3 route renders real data and Home fetches only index.json', async () => {
    ...
    try {
      ...
      const { clearDataCache, playerBucket } = await import('../public/js/data.js?v=0.3.3');
      ...
      for (const hash of renderedRoutes) {
        ...
        if (hash === '#/p/Q27067753') {
          ...
          equal(images.some((image) => image.getAttribute('src')?.endsWith('Q27067753.webp?v=0.3.3')), true, 'Kubo player photo renders');
  ...
  test('release strings, cache versions, and minimum text sizes are consistent', () => {
    ...
    equal(readFileSync(join(ROOT, 'public/js/version.js'), 'utf8').includes("VERSION = '0.3.3'"), true, 'footer version');
